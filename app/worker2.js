...
class MyTextToSpeechPipeline {
    ...
    static async getInstance(progress_callback = null) {
        if (this.tokenizer_instance === null) {
            this.tokenizer_instance = AutoTokenizer.from_pretrained(this.model_id, { progress_callback });
        }
...
self.addEventListener('message', async (event) => {
    ...
    // Tokenize the input
    const { input_ids } = tokenizer(event.data.text, { padding: true, truncation: true });
    console.log(input_ids); 

    // Load the speaker embeddings
    let speaker_embeddings = speaker_embeddings_cache.get('cmu_us_bdl_arctic-wav-arctic_a0003');
    if (speaker_embeddings === undefined) {
        speaker_embeddings = await MyTextToSpeechPipeline.getSpeakerEmbeddings('cmu_us_bdl_arctic-wav-arctic_a0003');
        speaker_embeddings_cache.set('cmu_us_bdl_arctic-wav-arctic_a0003', speaker_embeddings);
    }
